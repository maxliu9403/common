...
func (sf *MyStructFields) Add(fields ...ColumnField) {
	for _, columnField := range fields {
		...
		if fieldKind == reflect.Ptr {
			fieldKind = columnField.Field.Type.Elem().Kind()
		}
		switch {
		default:
...
func JsonTagColumnMapFromModel(m interface{}, db *gorm.DB) (fields map[string]string) {
	t := reflect.TypeOf(m)
	for t.Kind() == reflect.Ptr || t.Kind() == reflect.Slice || t.Kind() == reflect.Array {
		t = t.Elem()
	}
	...
	return JsonTagColumnMap(t, db)
}

// JsonTagColumnMap 模型的JsonTag对应的字段名称
func JsonTagColumnMap(t reflect.Type, db *gorm.DB) map[string]string {
	var jsonTagColumnMap = make(map[string]string)
	for i := 0; i < t.NumField(); i++ {
		if t.Field(i).Tag != "" {
			...
			if !reg.MatchString(gormTag) {
				continue
			}

			columnName := reg.FindStringSubmatch(gormTag)[1]
			jsonTagColumnMap[jsonTag] = columnName
		}
	}
	return jsonTagColumnMap
}
